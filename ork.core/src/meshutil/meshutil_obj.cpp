...
struct objpoly
{
	...
	std::vector<int>	mvtxindices;
	std::vector<int>	mnrmindices;
...
struct objmesh
{
	std::string			name;
	std::string			matname;
	std::vector<objpoly>	mpolys;
};
struct objmat
{
	CVector3			mColor;
};
void toolmesh::WriteToWavefrontObj( const Path& BasePath ) const
{
	ork::Path ObjPath = BasePath;
	ork::Path MtlPath = BasePath;
	...
	outstr += FormatString("mtllib %s.mtl\n", MtlPath.GetName().c_str() );

	///////////////////////////////////////////////////
	///////////////////////////////////////////////////
	std::vector<CVector3>			ObjVertexPool;
	std::vector<CVector3>			ObjNormalPool;
	std::vector<CVector2>			ObjUv0Pool;
	std::vector<objmesh>			ObjMeshPool;
	std::map<std::string,objmat>	ObjMaterialPool;
	///////////////////////////////////////////////////
	for( ork::orklut<std::string,submesh*>::const_iterator 
	...
	for( std::map<std::string,submesh*>::const_iterator 
	...
	for( std::map<std::string,objmat>::const_iterator itm=ObjMaterialPool.begin(); itm!=ObjMaterialPool.end(); itm++ )
	{
...
void toolmesh::ReadFromWavefrontObj( const Path& BasePath )
{
	ork::Path ObjPath = BasePath;
	ork::Path MtlPath = BasePath;
	ork::Path FxmPath = BasePath;
	...
	if( false == ObjPath.IsFile() )
		return;

	FILE* objfile = fopen( ObjPath.c_str(), "rt" );

	//objfile.Open(  );
	size_t ifilelen = ObjPath.SizeOfFile();
	char* pmem = new char[ ifilelen+1 ];
	fread( pmem, ifilelen, 1, objfile  );
	fclose( objfile );
	pmem[ifilelen] = 0;
	...
	std::vector<std::string> ObjLines = ork::SplitString( InObjText, '\n' );

	int inumlines = int(ObjLines.size());

	std::vector<CVector3> ObjV;
	std::vector<CVector3> ObjVN;
	std::vector<CVector2> ObjVT;
	std::vector<objpoly>  ObjPolys;
	...
	for( int il=0; il<inumlines; il++ )
	{
		...
		std::vector<std::string> Tokens = ork::SplitString( curline, ' ' );
			...
			switch( firstch )
			{
				...
				{	OrkAssert( Tokens[0] == "f" );
					...
					for( int ivtx=0; ivtx<inumfaceverts; ivtx++ )
					{
						...
						std::vector<std::string> indices = ork::SplitString( tok, '/' );
